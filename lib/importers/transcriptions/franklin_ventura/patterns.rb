module Patterns
    Holder_extractor = /(?<before>\(|; |and )<F53621>(?<loc_code>(a|h|bpl|y-brbl|y-mssa)) ?(?<subloc_code>(b|h|l|st|mlt|mtb|pc|to|tr))?<F255>( (1896)?<F53621>(?<subloc_code>(b|h|l|st|mlt|mtb|pc|to|tr))<F255>(, )?)?(<N>)? ?(?<id>[^\)<;]+)(?<after>\)|;| and)/
