class MissingImageCreator
    ...
    def imageless_works
        Work.joins("INNER JOIN setts AS s1
                   ON s1.id = works.image_set_id AND s1.type = 'ImageSet'
                   LEFT OUTER JOIN setts AS s2 ON s2.ancestry = CAST(s1.id AS text)"
                  ).where(s2: { nestable_id: nil }).readonly(false)
    end
end
